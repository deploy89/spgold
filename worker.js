...
async function api(req,env,u){const p=u.pathname.replace(/^\/api\/?/,'').split('/').filter(Boolean);const method=req.method;
  ...
  if(p[0]==='auth'&&p[1]==='register'&&method==='POST'){const b=await body(req);if(badKeys(b)||!b.name||!b.phone||!b.password||String(b.password).length<4)return json({ok:false,error:'validation failed'},400);const phone=String(b.phone).trim();if(!/^\d{10}$/.test(phone))return json({ok:false,error:'หมายเลขโทรศัพท์ไม่ถูกต้อง กรุณากรอก 10 หลัก'},400);const ex=await env.DB.prepare('SELECT id FROM users WHERE phone=?').bind(phone).first();if(ex)return json({ok:false,error:'phone already registered'},409);const h=await bcrypt.hash(String(b.password),10),uid=phone,t=now(),uc=await genUserCode(env),data={id:uid,name:String(b.name).trim(),phone,user_code:uc,balance:0,vip:1,kycStatus:'none',avatar:null,createdAt:t};await env.DB.prepare('INSERT INTO users(id,phone,password_hash,data,created_at) VALUES(?,?,?,?,?)').bind(uid,phone,h,JSON.stringify(data),t).run();const l=await login(env,'user',phone,String(b.password));await notify(env,'register',uid,'สมาชิกใหม่',`${data.name} สมัครสมาชิก`);return json({ok:true,user:data},201,{'set-cookie':cookie(sessionCookieName('user'),l.token,2592000,cookieSecure(req))})}
  if(p[0]==='auth'&&p[1]==='login'&&method==='POST'){const b=await body(req);const phone=String(b.phone||'').trim();if(!/^\d{10}$/.test(phone))return json({ok:false,error:'หมายเลขโทรศัพท์ไม่ถูกต้อง'},400);const fk='login:user:'+phone;const blocked=tooManyLoginFails(fk);if(blocked)return blocked;const l=await login(env,'user',phone,String(b.password||''));if(!l){recordFail(fk,LOGIN_FAIL_WINDOW);return json({ok:false,error:'invalid credentials'},401);}clearFails(fk);return json({ok:true,user:safeUser(l.user)},200,{'set-cookie':cookie(sessionCookieName('user'),l.token,2592000,cookieSecure(req))})}
  ...
  if(p[0]==='admin'&&p[1]==='chats'&&p[2]&&method==='DELETE'){if(!admin)return json({ok:false,error:'forbidden'},403);const mid=String(p[2]);const r=await env.DB.prepare('DELETE FROM chats WHERE id=?').bind(mid).run();await audit(env,'admin:'+admin.actor_id,'delete_chat_msg',mid,{},req.headers.get('CF-Connecting-IP'));return json({ok:true,deleted:r.meta?.changes||0})}
